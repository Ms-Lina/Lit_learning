...
async def register_user(db:db_dependency, create_user_request: CreateUserRequest):
    try:
        ...
        create_user_model = Users(
            ...
            user_type=create_user_request.user_type,
        )
...
async def login_for_access_token(form_data: FromData, db:db_dependency):
    ...
    return {"UserInfo": user_info, "access_token": token, "token_type": "bearer", "user_type": user.user_type,"email":user.email,"username":user.username,"id":user.id}
